...
class GAIN(object):
    ...
    def _compute_gradients(self, z, m, h):
        with tf.GradientTape() as d_tape, tf.GradientTape() as g_tape:
            ...
            # hat_g = z * m + g_out * (1 - m)
            d = self.discriminator([g_out, h], training=True)
    ...
    @staticmethod
    def _get_mask_vector2(x, idx):
        m = np.ones(shape=x.shape)
        m[idx] = 0

        return m

    def _get_hint_vector(self, m, idx, value=0.5):
        h_1 = value * (np.random.uniform(0., 1., size=m.shape) > 1 - self.p_hint)
    ...
    def _plot_validation(self, x, iteration, log_dir):
        ...
        missing_idx = np.isnan(x_batch)
        m_batch = self._get_mask_vector2(x_batch, missing_idx)

        z_batch = x_batch.copy()
        z_batch[missing_idx] = self._get_random_noise(x_batch)[missing_idx]
        ...
        z_batch[missing_idx] = self._get_random_noise(x_batch)[missing_idx]
        samples1 = self.generator([z_batch, m_batch], training=False)

        z_batch[missing_idx] = self._get_random_noise(x_batch)[missing_idx]
        samples2 = self.generator([z_batch, m_batch], training=False)

        z_batch[missing_idx] = self._get_random_noise(x_batch)[missing_idx]
        samples3 = self.generator([z_batch, m_batch], training=False)
    ...
    def fit(self, x, validation=None, iterations=10000, log_dir=None):
        ...
        with trange(iterations) as tqdm_range:
            for iteration in tqdm_range:
                ...
                z_batch = x_batch.copy()
                z_batch[missing_idx] = self._get_random_noise(x_batch)[missing_idx]
                h_batch = self._get_hint_vector(m_batch, missing_idx)
...
if __name__ == '__main__':
    ...
    masking_scaled_x_train = masking_image(scaled_x_train)
    masking_scaled_x_test = masking_image(scaled_x_test)

    masking_scaled_x_train = masking_scaled_x_train.reshape(
        masking_scaled_x_train.shape[0], masking_scaled_x_train.shape[1] * masking_scaled_x_train.shape[2]
    )
    masking_scaled_x_test = masking_scaled_x_test.reshape(
        masking_scaled_x_test.shape[0], masking_scaled_x_test.shape[1] * masking_scaled_x_test.shape[2]
    )

    gain_model = GAIN(input_shape=[masking_scaled_x_train.shape[1]], batch_size=128, alpha=50)
    gain_model.fit(masking_scaled_x_train, iterations=10000, validation=masking_scaled_x_test, log_dir="fashion_mnist")
